...
def submit_test():
    ...
    # Обновляем прогресс
    if correct:
        # При правильном ответе повышаем уровень слова
        repetition = Repetition.query.filter_by(word_id=test.word_id, user_id=user_id).first()
        if repetition:
            # Засчитываем правильный ответ как оценку 5
        ...
        progress = UserProgress.get_or_create_today(user_id=user_id)
        progress.correct_answers += 1
        progress.points += 15  # Больше очков за тест
        db.session.commit()
...
def new_test():
    ...
    if request.method == 'POST':
        ...
        test_session = create_test_session(test_type, difficulty, word_count)
...
def take_test_session(session_id):
    ...
    session_data = TestSession.query.get_or_404(session_id)
    ...
    word_ids = json.loads(session_data.words)
    current_position = session_data.current_position
    
    # Если все слова пройдены, завершаем сессию
    ...
    # Получаем или создаем тест для текущего слова
    test = Test.query.filter_by(word_id=current_word_id).first()
    if not test:
        test = create_test_for_word(current_word_id, difficulty=session_data.difficulty)
...
def submit_test_answer():
    ...
    # Получаем тестовую сессию
    session_data = TestSession.query.get(session_id)
    ...
    # Обновляем статистику тестовой сессии
    session_data.total_questions += 1
    if correct:
        session_data.correct_answers += 1
    
    # Переходим к следующему слову
    session_data.current_position += 1
    
    # Проверяем, завершен ли тест
    all_completed = session_data.current_position >= len(json.loads(session_data.words))
    if all_completed:
        session_data.complete_session()
        
        # Обновляем статистику пользователя
        progress = UserProgress.get_or_create_today(user_id=user_id)
        progress.points += session_data.score
...
def test_results(session_id):
    ...
    results = {
        ...
        'percent': int((session_data.correct_answers / session_data.total_questions) * 100) if session_data.total_questions > 0 else 0,
