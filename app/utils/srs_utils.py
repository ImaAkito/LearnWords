...
def update_streak():
    ...
    if yesterday_progress and today_progress.words_reviewed > 0:
        ...
        if today_progress.streak >= 3:
            today_progress.points += 20  # Бонус за streak
    elif today_progress.words_reviewed == 0:
        # Сбрасываем streak, если нет повторений сегодня
...
def _create_multiple_choice_test(word, difficulty):
    """Создает тест с множественным выбором"""
...
def create_test_session(session_type, difficulty=None, word_count=10, category_id=None):
    """Создает новую тестовую сессию с указанными параметрами
    
    Args:
        session_type (str): Тип сессии (daily, custom, thematic)
        difficulty (int): Сложность 1-5, None для автовыбора
        word_count (int): Количество слов в тесте
        category_id (int): ID категории для тематического теста
    """
    ...
    else:  # custom или любой другой тип
    ...
    test_session = TestSession(
        ...
        total_questions=len(word_ids),  # Общее количество вопросов = количество слов
        words=json.dumps(word_ids),
        correct_answers=0,  # Явно устанавливаем в ноль
        current_position=0  # Начинаем с первого слова
    )
...
def get_user_rank(points):
    """Определяет ранг пользователя на основе набранных очков"""
    if points < 100:
        return "Новичок"
    elif points < 500:
        return "Ученик"
